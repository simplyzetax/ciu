...
$(addprefix deploy-,$(FIRMWARES)): deploy-%: build-%
	$(ESPFLASH) flash --skip-update-check --baud $(FLASH_BAUD) $(ESPFLASH_PORT) \
		target/$(ESP_TARGET)/debug/ciu-$*

$(addprefix monitor-,$(FIRMWARES)): monitor-%: build-%
	$(ESPFLASH) monitor --skip-update-check $(ESPFLASH_PORT) \
		--monitor-baud $(MONITOR_BAUD) --elf target/$(ESP_TARGET)/debug/ciu-$*

$(addprefix deploy-monitor-,$(FIRMWARES)): deploy-monitor-%: build-%
	$(ESPFLASH) flash --skip-update-check --baud $(FLASH_BAUD) $(ESPFLASH_PORT) \
		--monitor --monitor-baud $(MONITOR_BAUD) target/$(ESP_TARGET)/debug/ciu-$*
